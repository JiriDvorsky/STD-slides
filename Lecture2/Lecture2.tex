...
\begin{frame}[fragile]
	\frametitle{Příkazy}
		\begin{itemize}
			...
			\item slangově se nazývají také jako \emph{makra},
			\item rozlišujeme dva typy příkazů:
				\begin{itemize}
					\item \emph{řídící slova} libovolně dlouhou sekvencí písmen anglické abecedy -- 
					\item \emph{řídící znaky} jediným nepísmeným znakem -- 
				\end{itemize}
			


			\item začínají vždy zpětným lomítkem |\| a pokračují
		\end{itemize}
\end{frame}
